  ...
  let indexRouter = [
    {
      ...
      meta: { title: '首页' },
      redirect: '/baseData/schoolManage',
...
/**
 * 比较字符串日期大小(减1000是为了同一天比较时，开始时间早于结束时间)
 */

export function compareDateCopy(sTime, eTime) {
  return new Date(sTime.replace(/-/g, '/')) - 1000 < new Date(eTime.replace(/-/g, '/'))
}
...
export function editDateForSearch(obj) {
  ...
  if (obj.beginTime && obj.endTime) {
    if (!compareDate(obj.beginTime, obj.endTime)) {
      return false
    }
  }
  return obj
...
/**
 * 用于时间日期条件搜索编辑,可以比较同一天
 */
export function editDateForSearchCopy(obj) {
  obj.beginTime && (obj.beginTime = moment(obj.beginTime).format('YYYY-MM-DD'))
  obj.endTime && (obj.endTime = moment(obj.endTime).format('YYYY-MM-DD'))
  console.log('obj.endTime', obj.endTime)
  if (obj.beginTime && obj.endTime) {
    if (!compareDateCopy(obj.beginTime, obj.endTime)) {
      return false
    }
  }
  return obj
...
export function editDateForSearch_2(obj) {
  obj.beginTime && (obj.beginTime = new Date(moment(obj.beginTime).format('YYYY-MM-DD')))
  obj.endTime && (obj.endTime = new Date(moment(obj.endTime).format('YYYY-MM-DD')))
