  ...
  methods: {
    ...
    nodeClick(item) {
      if (item.type == 3 && [1, 2, 3].indexOf(item.key) !== -1) {
        ...
        let content =
          '利用最新的电测化技术手段，并结合互联网+通信平台，加强对边坡滑坡体的监测和预警，对于保障交通通行，减少直接经济损失和人员伤亡具有重要实际应用价值。'
        if (item.name == '仪器使用') {
          content =
            '首先需要先按下角度测量按钮，再去顺次对准相应的两个目标，要注意设置好水平盘上的数字。如果是距离测量，就需要棱镜的常数以及气温和气压等数值，完成数据输入后就可以进行测量了。'
        } else if (item.name != '自动化监测') {
          content =
            '如需观测某特定方向上的水平位移PP，在距离监测区域- -定距离以外选定 工作基点A，水平位移监测点的布设应尽量与工作基点在一- 条直线上。在一定远处(施工影响范围之外)选定一个控制点B，作为零方向。在B点安置觇牌，用测回法观测水平角∠BAP，测定一段时间内观测点与基准点连线与零方向之间的角度变化值，根据公式计算得出水平位移量。'
        }
        this.$info({
          title: item.name,
